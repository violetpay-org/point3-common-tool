...
이 패키지는 GitLab Package Registry를 통해 배포됩니다.

### 설치

프로젝트의 `.npmrc`에 레지스트리를 추가하세요:

```ini
@point3:registry=https://git.point3.io/api/v4/groups/264/-/packages/npm/
//git.point3.io/api/v4/groups/264/-/packages/npm/:_authToken=${GITLAB_NPM_TOKEN}
```
...
`v*` 태그가 push되면 CI/CD 파이프라인이 자동으로 GitLab Package Registry에 배포합니다.
...
- [GitLab Issues](https://git.point3.io/point3/point3-common-tool/-/issues)에서 문의 및 버그 제보를 해주세요.
